...
def monte_carlo_bisec(scenario, n,p,s,rho=None, cant_clusters = None,cant_sim = 1, eps = 0.05):
...
def grafico_montecarlo_bisec(scenario, n_list, p, s, rho=None,cant_clusters= None, cant_sim = 1, eps = 0.05, showfig = False, savefig = False, save_in = None):

    if scenario == '1' or scenario == '3':
        true_LASSOAREAS_nlist = []
        false_LASSOAREAS_nlist = []
        for n in n_list:
            mean_false_LASSOAREAS, mean_true_LASSOAREAS = monte_carlo_bisec(scenario,n,p,s,rho,cant_sim = cant_sim, eps = eps)
        ...
        if savefig:
            filename = save_in + '\SCE%s_LASSO_AR_s%s_rho%s_cant_sim%s.png' % (scenario, s, rho, cant_sim)
            plt.savefig(fname=filename)
        if showfig:
            plt.show()
    else:
        for n in n_list:
            mean_false_LASSOAREAS, mean_true_LASSOAREAS = monte_carlo_bisec(scenario, n,p,s,rho=rho, cant_clusters =cant_clusters,cant_sim = cant_sim, eps = 0.05)
            ...
            if savefig:
                filename = save_in + '\SC%s_LASSO_AR_n=%s_p=%s_s=%s_rho=%s_cantsim=%s.png' % (
...
cant_clusters = 10
cant_sim = 1000
save_in = r'C:\Vero\ML\codigos_Python\Figuras_paper\LASSOARBIS\SCENARIO%s' %(scenario)
for rho in rho_list:
    grafico_montecarlo_bisec(scenario, n_list, p, s, rho=rho,cant_clusters= cant_clusters, cant_sim = cant_sim, eps = 0.05, showfig = False, savefig = True, save_in = save_in)
